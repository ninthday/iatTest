...
<?php }?>
var iatItem2 = ['重視','親近','喜愛','珍惜','獲得','捨棄','遠離','厭惡','拒絕','排斥'];
var iatResponse2 = ['l','l','l','l','l','r','r','r','r','r'];
var iatTrialcode2 = ['need-l','need-l','need-l','need-l','need-l','NONneed-r','NONneed-r','NONneed-r','NONneed-r','NONneed-r'];
var lastItemNum, startTime;		//宣告儲存上次亂數值、計時起始時間
var maxTrialnum = 45;							//字詞最多出現次數
	 		...
	 		if (chkResponse(lastItemNum,e.keyCode)){
				...
				if(!wrongSymbol){
					...
					if(itemSwitch){
						...
						stimulus[arrayIndex] = iatItem2[lastItemNum];
						trialcode[arrayIndex] = iatTrialcode2[lastItemNum];
					}
					wrongSymbol = false;
				}
...
function save2Cookie(strBlockContent){
	...
	setCookies('block04',strBlockContent,expireDay);
}
...
<div id="IAT_area">
	<div id="top">
	<span id="topleft"><?php echo ($StuID%2)?'綠能':'核能'; ?></span><span id="topright"><?php echo ($StuID%2)?'核能':'綠能'; ?></span>
	</div>
    <div id="bottom">
	<span id="bottomleft">喜好</span><span id="bottomright">討厭</span>
	</div>
...
<form id="frmUploadData04" name="frmUploadData04" method="post" action="ajax_save_uploaddata.php">
    <input type="hidden" name="BlockArea" value="Block4" />
